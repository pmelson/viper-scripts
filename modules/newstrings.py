...
USERAGENT_REGEX = re.compile('Mozilla\/[0-9]\.0[a-zA-Z0-9\,\-\ \;\.\(\)\:\/]+', re.IGNORECASE)
...
class Strings(Module):
    ...
    def __init__(self):
        ...
        self.parser.add_argument('-b', '--browser', action='store_true', help='Extract browser User-Agent from strings')
        self.parser.add_argument('-H', '--hosts', action='store_true', help='Extract IP addresses and domains from strings')
        self.parser.add_argument('-U', '--url', action='store_true', help='Extract URL patterns from strings')
        self.parser.add_argument('-X', '--xorurl', action='store_true', help='Extract XOR encoded URL patterns from strings')
    ...
    def xordata(data, key):
        ...
        for i in range(len(encoded)):
            decoded += chr(encoded[i] ^ ord(key[i % l]))
        return decoded
    ...
    def extract_useragents(self, strings):
        results = []
        for entry in strings:
            to_add = False
